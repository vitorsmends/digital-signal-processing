...
def apply_highpass_filter(signal, cutoff_freq, sample_rate, filter_order=4):
    nyquist_rate = sample_rate / 2
    normalized_cutoff_freq = cutoff_freq / nyquist_rate
    b, a = iirfilter(filter_order, normalized_cutoff_freq, btype='highpass', analog=False, ftype='butter')
...
def apply_highpass_filter_to_audio_file(input_file, output_file, cutoff_freq, filter_order=4):
    audio = AudioSegment.from_file(input_file)
    sample_width = audio.sample_width
    sample_rate = audio.frame_rate
    channels = audio.channels
    signal = np.array(audio.get_array_of_samples())
    filtered_signal, b, a = apply_highpass_filter(signal, cutoff_freq, sample_rate, filter_order)
...
output_file = "./output-data/output_highpass_iir.mp3"
cutoff_freq = 500  # Adjust cutoff frequency as needed
filter_order = 4  # Adjust filter order as needed

apply_highpass_filter_to_audio_file(input_file, output_file, cutoff_freq, filter_order)
